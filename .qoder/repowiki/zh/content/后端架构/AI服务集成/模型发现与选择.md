...
## 更新摘要
**变更内容**
- DeepSeek提供商优先级显著提升：在PROVIDER_PRIORITY列表中位列第一，体现其在OPC场景中的成本效益和中英双语能力优势
- 新增DeepSeek特定的模型偏好推荐：在MODEL_PREFERENCES中添加deepseek-chat、deepseek-reasoner、deepseek-coder等专用模型
- 改进的提供商优先级列表：重新排序的供应商优先级，更好地平衡国际和中国AI提供商
- 前端显示顺序优化：DeepSeek在API密钥配置界面中优先显示，提升用户体验
...
- 模型ID优先级规则：按提供商优先级与模型偏好模式进行优选，现已包含DeepSeek的最高优先级。
- 默认模型配置策略：通过数据库记录的默认模型槽位进行统一管理。
- large_context 模型的选择逻辑、token 计数计算方法与模型类型验证机制。
- 模型配置检查、错误处理与日志记录实现，包括DeepSeek等提供商的特殊处理。
- 最佳实践、性能优化建议与故障排除指南。
...
graph TB
subgraph "AI 层"
MD["model_discovery.py<br/>模型发现与分类<br/>DeepSeek优先级提升"]
...
subgraph "API 层"
MR["routers/models.py<br/>模型API与自动分配<br/>DeepSeek最高优先级"]
...
FR["frontend/page.tsx<br/>前端显示优化<br/>DeepSeek优先显示"]
end
MD --> MM
PR --> MM
PR --> TU
...
**图表来源**
- [model_discovery.py](file://open_notebook/ai/model_discovery.py#L1-L1071)
...
## 核心组件
- 模型发现与分类：从各提供商拉取可用模型列表，并按名称模式分类为语言、嵌入、语音识别或语音合成模型。现已支持DeepSeek等多家提供商。
...
- API 自动分配：按提供商优先级与模型偏好模式，自动为各类默认槽位分配最优可用模型，现已包含DeepSeek的最高优先级。
...
## 架构总览
模型发现与选择贯穿"发现—登记—配置—实例化—调用"的完整链路，同时在运行时依据输入规模动态选择合适模型。DeepSeek的优先级提升确保系统在OPC场景中能够优先选择成本效益更高的模型。
...
- 功能要点
  ...
  - **增强**：DeepSeek提供完整的动态API发现机制，支持deepseek-chat、deepseek-reasoner、deepseek-coder等专用模型。
...
- 优先级规则
  ...
  - MODEL_PREFERENCES 定义各提供商内的偏好模型名称模式，**新增**DeepSeek特定推荐：
...
- 前端显示优化
  - **优先显示**：在API密钥配置界面中DeepSeek优先显示，提升用户体验
  - **静态模型列表**：当API发现失败时提供静态模型回退
- 配置支持
  - 通过环境变量 `DEEPSEEK_API_KEY` 进行配置
  - 支持凭据注入和环境变量回退
...
- 模块耦合
  ...
  - **增强**：DeepSeek优先级提升影响整个自动分配流程，使其成为首选提供商。
- 外部依赖
...
- 模型发现
  ...
  - **增强**：DeepSeek优先级提升减少不必要的API调用，提高选择效率。
- token 计数
...
## 结论
本机制通过"发现—登记—配置—实例化—运行时选择"的闭环，实现了灵活、可扩展且健壮的模型管理与选择能力。**DeepSeek优先级的显著提升**进一步增强了系统的智能化服务能力，通过最高优先级的提供商选择和专用模型推荐，确保系统在OPC场景中能够优先选择成本效益更高、性能更优的模型。结合完整的API自动分配与完善的日志/异常体系，便于运维与排错，为用户提供更加高效、可靠的AI模型服务体验。