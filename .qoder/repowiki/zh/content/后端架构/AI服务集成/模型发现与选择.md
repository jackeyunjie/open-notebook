...
- [page.tsx（前端）](file://frontend/src/app/(dashboard)/settings/api-keys/page.tsx)
- [ModelSelector.tsx](file://frontend/src/components/common/ModelSelector.tsx)
- [use-models.ts](file://frontend/src/lib/hooks/use-models.ts)
- [models.ts](file://frontend/src/lib/api/models.ts)
</cite>

## 更新摘要
**变更内容**
- 新增Qwen 3.5 Plus模型支持：在通义千问模型系列中新增qwen3.5-plus模型，作为最新的Qwen 3.5系列旗舰模型
- 扩展Qwen模型覆盖范围：Qwen_MODEL_TYPES中新增"qwen3.5"前缀模式，支持Qwen 3.5系列的所有变体
- 更新模型偏好配置：MODEL_PREFERENCES中新增Qwen 3.5系列的偏好模型，包括qwen3.5-plus等旗舰型号
- 前端显示优化：Qwen 3.5 Plus在模型选择界面中优先显示，提升用户体验
...
- 模型ID优先级规则：按提供商优先级与模型偏好模式进行优选，现已包含DeepSeek的最高优先级和Qwen 3.5 Plus的最新支持。
- 默认模型配置策略：通过数据库记录的默认模型槽位进行统一管理。
- large_context 模型的选择逻辑、token 计数计算方法与模型类型验证机制。
- 模型配置检查、错误处理与日志记录实现，包括DeepSeek、Qwen 3.5 Plus等提供商的特殊处理。
- 最佳实践、性能优化建议与故障排除指南。
...
- 异常与日志：统一异常体系与日志记录
- DeepSeek优先级增强：新增DeepSeek特定的模型推荐和优先级提升
- Qwen 3.5 Plus支持：新增通义千问3.5系列的最新模型支持

```mermaid
graph TB
subgraph "AI 层"
MD["model_discovery.py<br/>模型发现与分类<br/>DeepSeek优先级提升<br/>Qwen 3.5 Plus支持"]
...
subgraph "API 层"
MR["routers/models.py<br/>模型API与自动分配<br/>DeepSeek最高优先级<br/>Qwen 3.5 Plus偏好"]
...
KP["key_provider.py<br/>提供商配置支持"]
FR["frontend/page.tsx<br/>前端显示优化<br/>DeepSeek优先显示<br/>Qwen 3.5 Plus优先"]
SEL["frontend/ModelSelector.tsx<br/>模型选择组件"]
HOOK["frontend/use-models.ts<br/>模型查询钩子"]
API["frontend/models.ts<br/>前端API封装"]
end
MD --> MM
PR --> MM
PR --> TU
...
**图表来源**
- [model_discovery.py](file://open_notebook/ai/model_discovery.py#L1-L1073)
...
- [models.py（路由）](file://api/routers/models.py#L1-L782)
...
- [key_provider.py](file://open_notebook/ai/key_provider.py#L316-L465)
- [page.tsx（前端）](file://frontend/src/app/(dashboard)/settings/api-keys/page.tsx#L85-L93)
- [ModelSelector.tsx](file://frontend/src/components/common/ModelSelector.tsx#L1-L67)
- [use-models.ts](file://frontend/src/lib/hooks/use-models.ts#L1-L200)
- [models.ts](file://frontend/src/lib/api/models.ts#L1-L106)
...
## 核心组件
- 模型发现与分类：从各提供商拉取可用模型列表，并按名称模式分类为语言、嵌入、语音识别或语音合成模型。现已支持DeepSeek、Qwen 3.5 Plus等多家提供商。
...
- API 自动分配：按提供商优先级与模型偏好模式，自动为各类默认槽位分配最优可用模型，现已包含DeepSeek的最高优先级和Qwen 3.5 Plus的最新支持。
...
- [models.py（路由）](file://api/routers/models.py#L634-L782)

## 架构总览
模型发现与选择贯穿"发现—登记—配置—实例化—调用"的完整链路，同时在运行时依据输入规模动态选择合适模型。DeepSeek的优先级提升和Qwen 3.5 Plus的支持确保系统在OPC场景中能够优先选择成本效益更高的模型。
...
U->>API : 请求模型列表/自动分配
API->>DISC : 发现提供商模型(DeepSeek优先, Qwen 3.5 Plus支持)
DISC->>DB : 同步注册新模型
API->>CFG : 读取默认模型槽位
...
### 组件A：模型发现与分类（已更新）
- 功能要点
  - 针对多家提供商（OpenAI、Anthropic、Google、Ollama、Groq、Mistral、DeepSeek、xAI、OpenRouter、Voyage、ElevenLabs、OpenAI-Compatible、Qwen等）分别实现发现函数。
  - **增强**：DeepSeek提供完整的动态API发现机制，支持deepseek-chat、deepseek-reasoner、deepseek-coder等专用模型；**新增**Qwen 3.5 Plus支持，包括qwen3.5-plus等最新旗舰型号。
...
ParseResp --> Classify["按名称模式分类模型类型<br/>Qwen 3.5 Plus支持"]
...
- [model_discovery.py](file://open_notebook/ai/model_discovery.py#L632-L724)
- [model_discovery.py](file://open_notebook/ai/model_discovery.py#L80-L108)

**章节来源**
- [model_discovery.py](file://open_notebook/ai/model_discovery.py#L135-L166)
- [model_discovery.py](file://open_notebook/ai/model_discovery.py#L589-L630)
- [model_discovery.py](file://open_notebook/ai/model_discovery.py#L632-L724)
- [model_discovery.py](file://open_notebook/ai/model_discovery.py#L663-L748)
- [model_discovery.py](file://open_notebook/ai/model_discovery.py#L80-L108)
...
### 组件D：API 自动分配默认模型（已更新）
- 优先级规则
  - **更新**：PROVIDER_PRIORITY 定义提供商优先顺序，现已将DeepSeek列为最高优先级：
    - deepseek、openai、anthropic、google、moonshot、qwen、zhipu、mistral、groq、xai、openrouter、ollama、azure、openai_compatible
  - MODEL_PREFERENCES 定义各提供商内的偏好模型名称模式，**新增**DeepSeek和Qwen 3.5 Plus特定推荐：
    - deepseek: ["deepseek-chat", "deepseek-reasoner", "deepseek-coder"]
    - qwen: ["qwen-max", "qwen-plus", "qwen3.5-plus", "qwen-coder-plus", "qwen2.5-72b-instruct"]
  - 其他提供商偏好保持不变，如gpt-4o、claude-3-5-sonnet等。
- 选择逻辑
...
R->>Q : 查询所有模型(按类型分组)
R->>SEL : 为每个槽位选择最佳模型(DeepSeek优先, Qwen 3.5 Plus支持)
...
**图表来源**
- [models.py（路由）](file://api/routers/models.py#L678-L782)
- [models.py（路由）](file://api/routers/models.py#L634-L676)

**章节来源**
- [models.py（路由）](file://api/routers/models.py#L87-L116)
- [models.py（路由）](file://api/routers/models.py#L634-L782)
...
### 组件F：DeepSeek优先级增强与Qwen 3.5 Plus支持（新增）
- DeepSeek优先级提升
  - **最高优先级**：在PROVIDER_PRIORITY列表中位列第一，体现其在OPC场景中的成本效益和中英双语能力优势
  - **专用模型推荐**：MODEL_PREFERENCES中新增deepseek-chat、deepseek-reasoner、deepseek-coder等专用模型
  - **动态API发现**：完整的DeepSeek API集成，支持实时模型列表获取
- **Qwen 3.5 Plus支持**
  - **最新旗舰模型**：在QWEN_MODELS中新增"qwen3.5-plus"作为Qwen 3.5系列的旗舰型号
  - **扩展模型覆盖**：QWEN_MODEL_TYPES中新增"qwen3.5"前缀模式，支持Qwen 3.5系列的所有变体
  - **偏好配置**：MODEL_PREFERENCES中新增qwen3.5-plus等Qwen 3.5系列的偏好模型
  - **静态模型列表**：当API发现失败时提供Qwen 3.5 Plus的静态模型回退
- 前端显示优化
  - **优先显示**：在API密钥配置界面中DeepSeek和Qwen 3.5 Plus优先显示，提升用户体验
  - **静态模型列表**：当API发现失败时提供静态模型回退
- 配置支持
  - 通过环境变量 `DEEPSEEK_API_KEY` 和 `DASHSCOPE_API_KEY` 进行配置
  - 支持凭据注入和环境变量回退

**章节来源**
- [models.py（路由）](file://api/routers/models.py#L87-L116)
- [model_discovery.py](file://open_notebook/ai/model_discovery.py#L498-L529)
- [model_discovery.py](file://open_notebook/ai/model_discovery.py#L80-L108)
- [page.tsx（前端）](file://frontend/src/app/(dashboard)/settings/api-keys/page.tsx#L85-L93)
- [page.tsx（前端）](file://frontend/src/app/(dashboard)/settings/api-keys/page.tsx#L165-L167)

### 组件G：前端模型选择组件（已更新）
- ModelSelector组件
  - 支持按模型类型过滤（language、embedding、speech_to_text、text_to_speech）
  - 显示模型名称和提供商信息
  - 集成React Query进行模型数据管理
- use-models钩子
  - 提供模型查询、创建、删除、默认配置等功能
  - 集成自动分配默认模型的mutate操作
- modelsApi封装
  - 提供完整的模型管理API调用方法
  - 支持模型发现、同步、测试等操作
...
- 模块耦合
  ...
  - **增强**：DeepSeek优先级提升和Qwen 3.5 Plus支持影响整个自动分配流程，使其成为首选提供商。
- 外部依赖
...
KP["key_provider.py"] --> MD
FR["frontend/page.tsx"] --> APIR
SEL["frontend/ModelSelector.tsx"] --> HOOK["frontend/use-models.ts"]
HOOK --> API["frontend/models.ts"]
```
...
- [exceptions.py](file://open_notebook/exceptions.py#L1-L71)
- [key_provider.py](file://open_notebook/ai/key_provider.py#L316-L465)
- [page.tsx（前端）](file://frontend/src/app/(dashboard)/settings/api-keys/page.tsx#L1-L1560)
- [ModelSelector.tsx](file://frontend/src/components/common/ModelSelector.tsx#L1-L67)
- [use-models.ts](file://frontend/src/lib/hooks/use-models.ts#L1-L200)
- [models.ts](file://frontend/src/lib/api/models.ts#L1-L106)
...
- 模型发现
  ...
  - **增强**：DeepSeek优先级提升和Qwen 3.5 Plus支持减少不必要的API调用，提高选择效率。
- token 计数
...
- **新增**："DeepSeek优先级问题"
  - 现象：期望DeepSeek但选择了其他提供商。
  - 排查：检查PROVIDER_PRIORITY配置、MODEL_PREFERENCES偏好设置；确认DeepSeek API密钥配置正确。
  - 参考：[models.py（路由）](file://api/routers/models.py#L87-L116)
- **新增**："Qwen 3.5 Plus模型不可用"
  - 现象：期望Qwen 3.5 Plus但选择了其他Qwen型号。
  - 排查：检查DASHSCOPE_API_KEY配置；确认Qwen 3.5 Plus在MODEL_PREFERENCES中的偏好设置；检查模型发现是否成功。
  - 参考：[model_discovery.py](file://open_notebook/ai/model_discovery.py#L80-L108)
  - 参考：[models.py（路由）](file://api/routers/models.py#L112)
...
## 结论
本机制通过"发现—登记—配置—实例化—运行时选择"的闭环，实现了灵活、可扩展且健壮的模型管理与选择能力。**DeepSeek优先级的显著提升和Qwen 3.5 Plus的最新支持**进一步增强了系统的智能化服务能力，通过最高优先级的提供商选择和最新旗舰模型推荐，确保系统在OPC场景中能够优先选择成本效益更高、性能更优的模型。结合完整的API自动分配与完善的日志/异常体系，便于运维与排错，为用户提供更加高效、可靠的AI模型服务体验。