...
## 更新摘要
**变更内容**
- 新增PowerShell恢复脚本（resume.ps1），为Windows开发者提供一键恢复开发环境的便利工具
- 新增开发工具文档，包含脚本使用说明和最佳实践
- 补充技能系统依赖安装脚本（install-deps.sh）的说明
- 增强文档导出工具（export_docs.py）的使用指南
- 完善开发环境快速恢复和环境设置的完整工具链
...
## 简介
本指南面向开发者与运维人员，提供 Open Notebook 的完整开发环境搭建流程，涵盖系统要求、Python 与 Node.js 版本、依赖安装、Docker 与单容器部署、环境变量与数据库初始化、AI 提供商密钥配置、开发服务器启动与热重载、技能系统开发支持、开发工具脚本使用、以及常见问题排查与优化建议。

## 项目结构
Open Notebook 采用前后端分离架构：后端使用 FastAPI（Python），前端使用 Next.js（TypeScript/React）。数据库为 SurrealDB；通过 uv 打包与管理 Python 依赖；Docker 多阶段构建镜像；Supervisor 统一管理多个进程；开发时支持本地直启与 Docker Compose 双模式，特别针对技能系统提供完整的热重载支持。新增的PowerShell恢复脚本为Windows开发者提供了一键恢复开发环境的便利工具。
...
- **PowerShell恢复脚本**：为Windows开发者提供一键恢复开发环境的便利工具。
...
## 架构总览
下图展示开发与生产两种典型部署形态：本地直启（开发机原生）与容器化（Docker + Supervisor），特别强调技能系统的热重载支持和PowerShell恢复脚本的使用。
...
C9["开发工具脚本<br/>install-deps.sh, export_docs.py"]
C1 --> C2
C2 --> C3
C2 --> C4
C2 --> C5
...
O --> G
P["PowerShell 恢复脚本<br/>scripts/resume.ps1"] --> K
Q["依赖安装脚本<br/>scripts/install-deps.sh"] --> B
R["文档导出工具<br/>scripts/export_docs.py"] --> S["doc_exports/"]
P --> Q
P --> R
```
...
## 结论
通过本指南，您可以在本地或容器环境中快速搭建 Open Notebook 的开发与生产环境。建议日常开发采用本地直启，配合 Docker Compose 进行集成测试；生产部署可选用单容器镜像以简化运维。新增的开发专用 Compose 配置、运行时配置系统、PowerShell 恢复脚本和开发工具集合进一步提升了开发体验，特别是技能系统的热重载支持和 Windows 开发者的环境恢复能力，显著改善了技能开发和日常开发的迭代效率。
