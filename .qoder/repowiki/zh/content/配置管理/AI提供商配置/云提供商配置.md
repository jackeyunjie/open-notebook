...
## 更新摘要
**变更内容**
- 新增前端模型选择系统的重大增强：新增17个AI提供商的预填充模型选择功能
- 更新提供商配置要点与建议，包含新增的中国提供商详细信息
- 增加中国提供商的成本估算、模型推荐和配置步骤
- 更新多提供商并行使用与切换指南，包含中国提供商选项
- 新增静态模型列表支持，提供API发现失败时的回退机制
...
**更新** 新增前端模型选择系统的重大增强：新增17个AI提供商的预填充模型选择功能，包括DeepSeek、阿里云百炼、硅基流动、智谱AI、月之暗面等提供商的完整模型列表，以及改进的默认基础URL配置支持。系统现在提供静态模型列表作为API发现失败时的回退机制，确保配置过程的可靠性。
...
F2["frontend/src/app/(dashboard)/settings/api-keys/page.tsx"]
F3["frontend/src/lib/api/models.ts"]
F4["frontend/src/lib/hooks/use-models.ts"]
F5["frontend/src/lib/types/models.ts"]
end
D1 --> S1
S1 --> S2
S1 --> S3
...
- [frontend/src/app/(dashboard)/settings/api-keys/page.tsx](file://frontend/src/app/(dashboard)/settings/api-keys/page.tsx#L1-L1560)
- [frontend/src/lib/api/models.ts](file://frontend/src/lib/api/models.ts#L1-L106)
- [frontend/src/lib/hooks/use-models.ts](file://frontend/src/lib/hooks/use-models.ts#L1-L200)
- [frontend/src/lib/types/models.ts](file://frontend/src/lib/types/models.ts#L1-L71)
...
- **新增** 静态模型列表：为17个AI提供商提供预填充的模型选择功能，包含完整的模型列表和默认基础URL配置。
- **新增** 前端模型选择：提供直观的模型选择界面，支持多选和搜索功能。
...
- [open_notebook/ai/provision.py](file://open_notebook/ai/provision.py#L1-L61)
- [open_notebook/utils/encryption.py](file://open_notebook/utils/encryption.py#L1-L199)
- [frontend/src/app/(dashboard)/settings/api-keys/page.tsx](file://frontend/src/app/(dashboard)/settings/api-keys/page.tsx#L163-L313)

## 架构总览
系统采用"凭证驱动 + 模型注册 + 动态选择"的架构模式。前端通过运行时配置确定API地址；后端通过凭证路由管理API密钥与端点；模型发现模块负责拉取并注册可用模型；默认模型选择模块根据上下文与配置返回最优模型实例。**更新** 新增静态模型列表支持，提供API发现失败时的回退机制。
...
- 针对不同提供商实现专用发现函数，如OpenAI、Anthropic、Google Gemini、Groq、Mistral、DeepSeek、xAI、OpenRouter、ElevenLabs、**阿里云百炼、硅基流动、智谱AI、月之暗面**等。
...
- **更新** 新增DeepSeek、阿里云百炼、智谱AI、月之暗面等提供商的测试模型配置。
...
### **新增** 前端模型选择系统
- **提供商排序**：中文服务商优先显示，DeepSeek排在首位，然后是阿里云百炼、硅基流动、智谱AI、月之暗面
- **静态模型列表**：为每个提供商提供预填充的模型选择功能，包含完整的模型列表
- **默认基础URL**：为每个提供商提供默认的基础URL配置
- **模型类型支持**：支持语言模型、嵌入模型、文本转语音、语音识别等多类型模型
- **回退机制**：当API发现失败时，使用静态模型列表作为回退方案

**章节来源**
- [frontend/src/app/(dashboard)/settings/api-keys/page.tsx](file://frontend/src/app/(dashboard)/settings/api-keys/page.tsx#L85-L313)
- [frontend/src/lib/api/models.ts](file://frontend/src/lib/api/models.ts#L1-L106)
- [frontend/src/lib/hooks/use-models.ts](file://frontend/src/lib/hooks/use-models.ts#L1-L200)
- [frontend/src/lib/types/models.ts](file://frontend/src/lib/types/models.ts#L1-L71)
...
- **新增** 前端模型选择依赖API接口（models.ts）和状态管理（use-models.ts）。
...
FrontendModels["frontend/src/app/(dashboard)/settings/api-keys/page.tsx"] --> ModelsAPI["frontend/src/lib/api/models.ts"]
ModelsAPI --> ModelsHooks["frontend/src/lib/hooks/use-models.ts"]
ModelsHooks --> ModelsTypes["frontend/src/lib/types/models.ts"]
```
...
- **新增** 静态模型列表：提供即时的模型选择体验，无需等待API响应。
...
- **新增** "模型发现失败"：系统会自动回退到静态模型列表，确保配置过程不受影响。
...
## 结论
通过统一的凭证管理、自动化的模型发现与注册、完善的连接测试与默认模型选择机制，系统为OpenAI、Anthropic、Google Gemini、Groq、OpenRouter、**阿里云百炼、硅基流动、智谱AI、月之暗面**等主流云提供商提供了标准化的配置流程。**更新** 新增的前端模型选择系统进一步提升了用户体验，提供17个AI提供商的预填充模型选择功能和静态模型列表支持，确保在API发现失败时仍能正常配置。结合多提供商并行使用与加密存储策略，既能满足不同场景下的质量与成本需求，又能保障安全性与可维护性。
...
- **DeepSeek**
  - 成本：约$0.0001–0.0005/1K tokens（取决于具体模型）
  - 推荐模型：deepseek-chat（对话）、deepseek-reasoner（推理）
  - 配置步骤：在设置中添加凭证，选择"DeepSeek"提供商，测试连接，发现并注册模型
  - 优势：支持推理增强模型，适合复杂任务
- **阿里云百炼（Qwen）**
  - 成本：约¥0.0005–0.002/1K tokens（人民币计价，约$0.00007–0.00028/1K tokens）
  - 推荐模型：qwen-max（高质量对话）、qwen-plus（平衡选择）、qwen-turbo（快速响应）
  - 配置步骤：在设置中添加凭证，选择"通义千问"提供商，测试连接，发现并注册模型
  - 优势：国内访问速度快，支持中文优化，价格相对便宜
- **硅基流动（SiliconFlow）**
  - 成本：约$0.0001–0.0005/1K tokens（取决于具体模型）
  - 推荐模型：Qwen/Qwen2.5-72B-Instruct（大模型）、Qwen/Qwen2.5-7B-Instruct（性价比）
  - 配置步骤：在设置中添加凭证，选择"SiliconFlow"提供商，测试连接，发现并注册模型
  - 优势：支持多种开源模型，适合多模型实验
- **智谱AI（Zhipu）**
  - 成本：约¥0.0003–0.0015/1K tokens（GLM系列）
  - 推荐模型：glm-5（最新最强）、glm-4-plus（平衡选择）、glm-4-flash（快速响应）
  - 配置步骤：在设置中添加凭证，选择"智谱AI"提供商，测试连接，发现并注册模型
  - 优势：GLM系列模型在中文理解方面表现优异
- **月之暗面（Moonshot）**
  - 成本：约¥0.0008–0.003/1K tokens（Kimi系列）
  - 推荐模型：kimi-k2-5（最新版本）、moonshot-v1-128k（长上下文）
  - 配置步骤：在设置中添加凭证，选择"Moonshot"提供商，测试连接，发现并注册模型
  - 优势：长上下文能力出色，适合处理长文档

**章节来源**
- [docs/5-CONFIGURATION/ai-providers.md](file://docs/5-CONFIGURATION/ai-providers.md#L25-L250)
- [open_notebook/ai/model_discovery.py](file://open_notebook/ai/model_discovery.py#L70-L125)
- [open_notebook/ai/model_discovery.py](file://open_notebook/ai/model_discovery.py#L216-L242)
- [open_notebook/ai/connection_tester.py](file://open_notebook/ai/connection_tester.py#L31-L37)
- [frontend/src/app/(dashboard)/settings/api-keys/page.tsx](file://frontend/src/app/(dashboard)/settings/api-keys/page.tsx#L163-L313)
...
- **新增** 支持静态模型列表回退，确保配置过程的可靠性。

**更新** 新增中国提供商的并行使用建议：
- 中国提供商（阿里云百炼、智谱AI、月之暗面）适合国内用户，具有更好的网络性能和成本优势
- 国际提供商（OpenAI、Anthropic、Google Gemini）适合国际化团队和多语言场景
- 可以在同一项目中混合使用，根据任务类型选择最适合的提供商
- **新增** 前端提供直观的模型选择界面，支持多选和搜索功能

**章节来源**
- [api/routers/credentials.py](file://api/routers/credentials.py#L319-L358)
- [open_notebook/ai/models.py](file://open_notebook/ai/models.py#L220-L264)
- [api/routers/models.py](file://api/routers/models.py#L87-L114)
- [frontend/src/app/(dashboard)/settings/api-keys/page.tsx](file://frontend/src/app/(dashboard)/settings/api-keys/page.tsx#L85-L313)
...
**更新** 新增中国提供商的安全建议：
- 中国提供商的API密钥存储遵循相同的安全标准
- 建议优先使用阿里云等国内托管服务的密钥管理功能
- 跨境数据传输需遵守相关法律法规
...
| 特性 | DeepSeek | 阿里云百炼 | 硅基流动 | 智谱AI | 月之暗面 |
|------|----------|------------|----------|--------|----------|
| **语言支持** | 多语言 | 中文优化 | 多语言 | 中文优化 | 中文优化 |
| **长上下文** | 支持（128K） | 支持（128K） | 支持（128K） | 支持（128K） | 支持（128K） |
| **推理能力** | 优秀 | 一般 | 一般 | 优秀 | 一般 |
| **多模态** | 图像理解 | 图像理解 | 图像理解 | 图像理解 | 无 |
| **嵌入模型** | 支持 | 支持 | 支持 | 支持 | 无 |
| **价格优势** | 较高 | 中等 | 较高 | 中等 | 中等 |
| **网络延迟** | 中等 | 低（国内） | 中等 | 低（国内） | 低（国内） |
| **静态模型列表** | ✅ | ✅ | ✅ | ✅ | ✅ |
...
- **提供商排序**：中文服务商优先显示，DeepSeek排在首位，然后是阿里云百炼、硅基流动、智谱AI、月之暗面
- **静态模型列表**：为每个提供商提供预填充的模型选择功能，包含完整的模型列表
- **默认基础URL**：为每个提供商提供默认的基础URL配置，简化配置过程
- **模型类型支持**：支持语言模型、嵌入模型、文本转语音、语音识别等多类型模型
- **回退机制**：当API发现失败时，使用静态模型列表作为回退方案
- **用户界面**：提供直观的模型选择界面，支持多选和搜索功能
